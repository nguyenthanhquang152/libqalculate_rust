...
**Issue #62 update**: `-f`, `-file`, and `--file` now execute command streams from a path or explicit stdin (`-f -`) without REPL prompts, input echo, or history persistence. One calculator session preserves settings, assignments, and typed answer history across lines; comments/blanks are skipped, invalid commands report and continue with upstream-compatible status, a trailing argv expression runs after file EOF, and `quit` suppresses both remaining lines and the trailing expression. Fallback-disabled stdin evidence and an exact `-nodefs` differential match upstream 5.11.0. Bare no-argument input remains the interactive #61 contract; `--test-file` batch semantics remain #63.

...
| 14 | `stats.batch` | 39 | 0 | `vectordata.csv`, `vectordata2.csv` | `scaffold`; selected literal `mean`/`stdev`/`quartile`/`percentile`/`normdist`/`normdistinv`/`quadraticfit`/`cubicfit`/`fdist`/`chisqdistinv`/`mode`/`median` rows are native-pass; direct CSV-backed `mean`/`stdev`/`min`/`max`/`total`/`range`/`median` over `load(tests/vectordata.csv)` are oracle-proven, including quoted-path forms, but the session-variable `stats.batch` rows remain inventory-only |
...
forms, under the same working-directory rules. The `stats.batch` rows that

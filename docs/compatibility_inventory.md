...
| 4 | Arithmetic | 15 | `scaffold` | `add`, `sub`, `mul`, `div`, `pow`, `sqrt`, `ln`, `negate`, and `abs` exist for selected variants; float `ln` and non-integer `pow` now use MPFR-backed arithmetic for the covered slice, division-by-zero declines native success, and full upstream edge-case parity remains incomplete |

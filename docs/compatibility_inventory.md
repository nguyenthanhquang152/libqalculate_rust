...
| 12 | `MathStructure-matrixvector.cc` | Matrix and vector operations | `scaffold` via `src/matrix.rs`; native-pass coverage is limited to vector/matrix literal construction, selected top-level list output, selected identity construction, selected shape/accessors, selected `adj`, `cofactor`, `combine`, `cross`, `det`, `dot` function/operator rows, selected `entrywise`, `genvector`, `horzcat`, `vertcat`, `inverse`, magnitude, `norm`, `part`, `permanent`, `pow`/entrywise power, `rank`, `rk`, `rref`, `slice`, `sort`, and `transpose` rows, scalar scaling/subtraction, selected multiply/divide arithmetic, selected `hadamard` entrywise multiplication rows, row/column elementwise broadcasting, one rectangular matrix multiplication row, same-shape elementwise multiplication rows, and selected entrywise power broadcasting rows |
...
| 5 | `BuiltinFunctions-matrixvector.cc` | Matrix/vector functions | `scaffold` via `src/matrix.rs` and `src/data.rs`; selected `vector`, `matrix`, `matrix2vector`, `columns`, `dimension`, `rows`, `row`, `column`, `element`, `elements`, `multiply`, `adj`, `cofactor`, `combine`, `cross`, `det`, `dot` function/operator rows, `entrywise`, `hadamard`, `horzcat`, `identity`, `magnitude`, `norm`, `part`, `permanent`, collection `pow`, `rank`, `rk`, `slice`, `sort`, `transpose`, `vertcat`, `divide`, and `rdivide` rows are native-pass; `load(...)` has a narrow CSV count proof for `vectordata.csv`/`vectordata2.csv` but not full vector/statistics consumption |
...
| 9 | `BuiltinFunctions-statistics.cc` | Statistical functions | `scaffold` via `src/statistics.rs`; selected literal `mean`/`stdev`/`quartile`/`percentile`/`normdist`/`normdistinv`/`quadraticfit`/`cubicfit`/`fdist`/`chisqdistinv`/`mode`/`median` rows are native-pass |
...
| 14 | `stats.batch` | 39 | 0 | `vectordata.csv`, `vectordata2.csv` | `scaffold`; selected literal `mean`/`stdev`/`quartile`/`percentile`/`normdist`/`normdistinv`/`quadraticfit`/`cubicfit`/`fdist`/`chisqdistinv`/`mode`/`median` rows are native-pass |
...
Rust and upstream `qalc` run from the upstream checkout. The `stats.batch` rows
that depend on loaded session variables remain inventory-only until native
session variable consumption and broader statistics functions are proven.

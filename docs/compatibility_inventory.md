...
| 5 | `BuiltinFunctions-matrixvector.cc` | Matrix/vector functions | `scaffold` via `src/matrix.rs`; selected `vector`, `matrix`, `matrix2vector`, `columns`, `dimension`, `rows`, `row`, `column`, `element`, `elements`, `multiply`, `hadamard`, `identity`, `magnitude`, `norm`, `part`, `divide`, and `rdivide` rows are native-pass |
...
| Native-pass batch cases | 244 |
| Inventory-only batch cases | 412 |

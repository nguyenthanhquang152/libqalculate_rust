...
| 9 | `BuiltinFunctions-statistics.cc` | Statistical functions | `scaffold` via `src/statistics.rs`; selected literal `mean`/`stdev`/`quartile`/`percentile`/`normdist`/`quadraticfit`/`cubicfit`/`mode`/`median` rows are native-pass |
...
| 14 | `stats.batch` | 39 | 0 | `vectordata.csv`, `vectordata2.csv` | `scaffold`; selected literal `mean`/`stdev`/`quartile`/`percentile`/`normdist`/`quadraticfit`/`cubicfit`/`mode`/`median` rows are native-pass |
...
| Native-pass batch cases | 311 |
| Inventory-only batch cases | 345 |

...
**Issue #62 update**: `-f`, `-file`, and `--file` now execute command streams from a path or explicit stdin (`-f -`) without REPL prompts, input echo, or history persistence. One calculator session preserves settings, variable/function definitions, assignments, and typed answer history across lines; recognized stream settings remain active even when the retained answer cannot be reformatted. Leading comments/blanks are skipped, expression comments retain upstream parsing semantics, invalid commands report and continue with upstream-compatible status, list actions retain upstream precedence over file execution, a trailing argv expression runs after file EOF, and `quit` suppresses both remaining lines and the trailing expression. Fallback-disabled stdin evidence and exact `-nodefs` differentials match upstream 5.11.0; user-function definition execution remains an explicitly bounded C++ session bridge rather than native function-body parity. Bare no-argument input remains the interactive #61 contract; batch-test execution is the #63 contract below.

**Issue #63 update**: `--test-file` now executes native batch items in one calculator session, preserving typed `set`/`assume` commands plus unasserted setup and cleanup expressions. It compares exact UTF-8 output (including joined multi-line expectations), stops at the first mismatch with upstream-compatible ANSI diagnostics and line numbers, emits upstream-compatible pass/zero-test summaries, and preserves missing-file/no-path exit behavior. Fallback-disabled exact differentials cover the stateful `/set base` fixture and upstream `parser.batch` cases `:1`, `:3`, `:5`, `:7`, and `:9`. This proves the CLI execution mode and those already-promoted native rows; it does not promote the remaining inventory-only batch cases.
...
| 13 | `--test-file <path>` | Run batch test file | Stateful native batch execution with exact comparisons and summaries | `native-pass` | Fallback-disabled exact differentials cover a stateful `/set base` fixture and `parser.batch:1`, `:3`, `:5`, `:7`, `:9`; unsupported inventory-only expressions fail rather than using C++ fallback |
...
| 15 | `-f`, `-file`, `--file` command stream | Execute commands from a path or `-` stdin | Prompt-free, stateful command-stream execution with optional trailing expression | `native-pass` | Exact upstream `-nodefs -t -f -` differential plus fallback-disabled path/stdin tests cover settings, answer history, comments, diagnostics, EOF, quit, aliases, and file-open status; `--test-file` uses the separate stateful #63 runner |

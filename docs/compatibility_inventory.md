...
also has fallback-disabled oracle coverage for direct one-vector consumers over
`load(tests/vectordata.csv)`: `mean`, `stdev`, `min`, `max`, `total`,
`range`, `median`, `geomean(abs(...))`, `harmmean(abs(...))`, `rms`,
`trimmean(..., 10)`, `winsormean(..., 10)`, `weighmean(...,
genvector(2;1;100))`, `stderr`, `meandev`, `quartile(..., 1, 7)`,
`percentile(..., 25, 7)`, `decile(..., 9, 7)`, and `iqr`, including
...
working-directory rules. The `stats.batch` rows that depend on loaded session
variables remain inventory-only until native session variable consumption is
proven.

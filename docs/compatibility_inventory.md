...
| 15 | `-f`, `-file`, `--file` command stream | Execute commands from a path or `-` stdin | Prompt-free, stateful command-stream execution with optional trailing expression | `native-pass` | Exact upstream `-nodefs -t -f -` differential plus fallback-disabled path/stdin tests cover settings, answer history, comments, diagnostics, EOF, quit, aliases, and file-open status; `--test-file` remains #63 |

...
| 9 | `BuiltinFunctions-statistics.cc` | Statistical functions | `scaffold` via `src/statistics.rs`; selected literal `mean`/`stdev`/`quartile`/`percentile`/`normdist`/`normdistinv`/`quadraticfit`/`cubicfit`/`fdist`/`chisqdistinv`/`mode`/`median` rows are native-pass, with additional direct CSV-backed `mean`/`stdev`/`min`/`max`/`total`/`range`/`median` over `load(tests/vectordata.csv)` and direct paired `pearson`/`spearman`/`covar`/`poolvar`/`ttest`/`pttest` over both upstream vector fixtures, including quoted-path forms |
...
|---|---|---|
| `scaffold` | MathStructure matrix/vector, MathStructure print, Number, BuiltinFunctions matrix/vector, selected statistical functions | 5 |
...
| 14 | `stats.batch` | 39 | 0 | `vectordata.csv`, `vectordata2.csv` | `scaffold`; selected literal `mean`/`stdev`/`quartile`/`percentile`/`normdist`/`normdistinv`/`quadraticfit`/`cubicfit`/`fdist`/`chisqdistinv`/`mode`/`median` rows are native-pass; direct CSV-backed `mean`/`stdev`/`min`/`max`/`total`/`range`/`median` over `load(tests/vectordata.csv)` and direct paired `pearson`/`spearman`/`covar`/`poolvar`/`ttest`/`pttest` over both upstream vector fixtures are oracle-proven, including quoted-path forms, but the session-variable `stats.batch` rows remain inventory-only |
...
| Native-pass batch cases | 315 |
| Inventory-only batch cases | 341 |
...
`number(load(tests/vectordata2.csv))`, including quoted-path forms, when both
Rust and upstream `qalc` run from the upstream checkout. `src/statistics.rs`
also has fallback-disabled oracle coverage for direct
`mean(load(tests/vectordata.csv))`, `stdev(load(tests/vectordata.csv))`,
`min(load(tests/vectordata.csv))`, `max(load(tests/vectordata.csv))`,
`total(load(tests/vectordata.csv))`, `range(load(tests/vectordata.csv))`,
and `median(load(tests/vectordata.csv))` consumers, including quoted-path
forms, plus direct `pearson`, `spearman`, `covar`, `poolvar`, `ttest`, and
`pttest` consumers over `load(tests/vectordata.csv)` and
`load(tests/vectordata2.csv)`, including quoted-path forms, under the same
working-directory rules. The `stats.batch` rows that
depend on loaded session variables remain inventory-only until native session
variable consumption and broader statistics functions are proven.

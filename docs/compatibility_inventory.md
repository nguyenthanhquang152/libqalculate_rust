...
forms, plus direct `pearson`, `spearman`, `covar`, and `poolvar` consumers
over `load(tests/vectordata.csv)` and `load(tests/vectordata2.csv)`, including
quoted-path forms, under the same working-directory rules. The `stats.batch` rows that

...
| 9 | `BuiltinFunctions-statistics.cc` | Statistical functions | `scaffold` via `src/statistics.rs`; selected literal `mean`/`stdev`/`quartile`/`percentile`/`normdist`/`quadraticfit`/`cubicfit`/`fdist`/`mode`/`median` rows are native-pass |
...
| 14 | `stats.batch` | 39 | 0 | `vectordata.csv`, `vectordata2.csv` | `scaffold`; selected literal `mean`/`stdev`/`quartile`/`percentile`/`normdist`/`quadraticfit`/`cubicfit`/`fdist`/`mode`/`median` rows are native-pass |
...
| Native-pass batch cases | 313 |
| Inventory-only batch cases | 343 |

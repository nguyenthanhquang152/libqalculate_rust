...
| 5 | `BuiltinFunctions-matrixvector.cc` | Matrix/vector functions | `scaffold` via `src/matrix.rs`; selected `vector`, `matrix`, `matrix2vector`, `columns`, `dimension`, `rows`, `row`, `column`, `element`, `elements`, `multiply`, `combine`, `hadamard`, `horzcat`, `identity`, `magnitude`, `norm`, `part`, `slice`, `vertcat`, `divide`, and `rdivide` rows are native-pass |
...
| Native-pass batch cases | 251 |
| Inventory-only batch cases | 405 |

...
| 5 | `BuiltinFunctions-matrixvector.cc` | Matrix/vector functions | `scaffold` via `src/matrix.rs`; selected `vector`, `matrix`, `matrix2vector`, `columns`, `dimension`, `rows`, `row`, `column`, `element`, `elements`, `multiply`, `adj`, `cofactor`, `combine`, `det`, `dot`, `hadamard`, `horzcat`, `identity`, `magnitude`, `norm`, `part`, `permanent`, collection `pow`, `rank`, `slice`, `sort`, `transpose`, `vertcat`, `divide`, and `rdivide` rows are native-pass |
...
| Native-pass batch cases | 280 |
| Inventory-only batch cases | 376 |

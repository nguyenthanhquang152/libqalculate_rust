...
| 8 | `DataSet.h` | DataSet, DataProperty, DataObject APIs | public `src/datasets.rs` catalog API | mixed: `native-pass`, `unstarted` | no | dataset category below |
...
| Parsing and structured evaluation | `parse`, `calculate`, `calculateAndPrint` | syntax-only `parse`; registry-aware `evaluate`, `calculate`, `calculate_and_print` | `native-pass` for implemented native AST/evaluator families; unsupported upstream semantics remain `unstarted`, never implicit fallback. |
| Result formatting | `print`, `calculateAndPrint`, `MathStructure::print` | `print`, `calculate_and_print`, precision/fraction/approximation setters | `native-pass` for plain native expression/number formatting; other `PrintOptions`, HTML/LaTeX, auto-format modes, and time-limited printing remain pending or `fallback-only`. |
...
| Dataset access | dataset loading and `getDataSet`/object/property lookup | public `datasets::load_dataset_catalog_from_dir` and typed `DatasetCatalog` APIs | `native-pass` for explicit XML catalog/object/property lookup; caller-owned dataset evaluation, arbitrary mutation, and persistence remain `unstarted`. |
...
| Supported numeric options | selected evaluation/print option fields and setters | precision, formatting approximation, and fraction-format getters/setters | `native-pass` only for the three fields consumed by native formatting; base/session commands and all other option interactions remain `unstarted` on the root façade. |

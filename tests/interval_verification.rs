...
fn test_division_by_interval_containing_zero() {
    ...
    // With the optimized division path for purely real divisors, uncertainty propagates to 10.0 +/- 20.0.
    let val = NumberValue::Float(Float::from_f64(1.0, prec));
    let unc = NumberValue::Float(Float::from_f64(2.0, prec));
    let div_unc = Number::new_uncertainty(val, unc.clone(), false);
    ...
    let div_unc_zero = Number::new_uncertainty(val_zero, unc, false);

...
fn test_bitwise_not() {
    // ~0 -> -1
    assert_eq!(
        eval_native("~0").unwrap(),
        Expression::Number(Number::from_i32(-1))
    );

    // ~-1 -> 0
    assert_eq!(
        eval_native("~-1").unwrap(),
        Expression::Number(Number::from_i32(0))
    );

    // ~-812 -> 811
    assert_eq!(
        eval_native("~-812").unwrap(),
        Expression::Number(Number::from_i32(811))
    );
...
fn test_bitwise_shifts_basic() {
    // 0 >> 0 -> 0
    assert_eq!(
        eval_native("0 >> 0").unwrap(),
        Expression::Number(Number::from_i32(0))
    );

    // 0 >> 1 -> 0
    assert_eq!(
        eval_native("0 >> 1").unwrap(),
        Expression::Number(Number::from_i32(0))
    );

    // 18 >> 2 -> 4
    assert_eq!(
        eval_native("18 >> 2").unwrap(),
        Expression::Number(Number::from_i32(4))
    );

    // 11 >> 0 -> 11
    assert_eq!(
        eval_native("11 >> 0").unwrap(),
        Expression::Number(Number::from_i32(11))
    );
    ...
    // -18 >> 1 -> -9
    assert_eq!(
        eval_native("-18 >> 1").unwrap(),
        Expression::Number(Number::from_i32(-9))
    );
    ...
    // 18 << 0 -> 18
    assert_eq!(
        eval_native("18 << 0").unwrap(),
        Expression::Number(Number::from_i32(18))
    );

    // 18 << 1 -> 36
    assert_eq!(
        eval_native("18 << 1").unwrap(),
        Expression::Number(Number::from_i32(36))
    );

    // -18 << 2 -> -72
    assert_eq!(
        eval_native("-18 << 2").unwrap(),
        Expression::Number(Number::from_i32(-72))
    );
}
...
fn test_bitwise_shifts_negative_amount() {
    ...
    assert_eq!(
        eval_native("5 >> -2").unwrap(),
        Expression::Number(Number::from_i32(20))
    );
...
fn test_bitwise_and_or_xor() {
    // 7 & 3 -> 3
    assert_eq!(
        eval_native("7 & 3").unwrap(),
        Expression::Number(Number::from_i32(3))
    );

    // 5 | 9 -> 13
    assert_eq!(
        eval_native("5 | 9").unwrap(),
        Expression::Number(Number::from_i32(13))
    );

    // 5 xor 9 -> 12
    assert_eq!(
        eval_native("5 xor 9").unwrap(),
        Expression::Number(Number::from_i32(12))
    );
...
fn test_bitwise_literal_spaces() {
    // 0b1011 0010 -> 178
    assert_eq!(
        eval_native("0b1011 0010").unwrap(),
        Expression::Number(Number::from_i32(178))
    );

    // 0b0111 0001 -> 113
    assert_eq!(
        eval_native("0b0111 0001").unwrap(),
        Expression::Number(Number::from_i32(113))
    );
}
...
fn test_bitwise_precedence() {
    ...
    assert_eq!(
        eval_native("5 | 9 xor 5 & 7").unwrap(),
        Expression::Number(Number::from_i32(13))
    );

    // 5 & 7 xor 9 | 5 -> 13
    assert_eq!(
        eval_native("5 & 7 xor 9 | 5").unwrap(),
        Expression::Number(Number::from_i32(13))
    );
}
